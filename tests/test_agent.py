...
from symphony.agent import ClaudePrintClient
from symphony.config import build_config
from symphony.models import Issue, JsonObject, WorkflowDefinition


@pytest.mark.asyncio
async def test_claude_print_client_passes_prompt_as_argument(tmp_path: Path) -> None:
    ...
    await ClaudePrintClient(config).run_turn(
        workspace=workspace,
